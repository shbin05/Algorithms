...
def bfs(room):
    ...
    while virus:
        ...
        for dr, dc in zip([r, r, r+1, r-1], [c+1, c-1, c, c]): # 상하좌우 탐색
            if 0 <= dr < N and 0 <= dc < M and not room[dr][dc]: # room 좌표 안에 있고 현재는 바이러스가 없을 경우
                room[dr][dc] = 2
                virus.append((dr, dc))
    
    cnt = sum(i.count(0) for i in room)
    
    global num
    num = max(num, cnt)

empty = [(r,c) for r in range(N) for c in range(M) if not room[r][c]] # 빈 칸인 좌표 list
